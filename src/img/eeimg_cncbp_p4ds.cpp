...
static const uint32_t swportmode[IDT_PORTCNT] = {SWPORTxCTL_MODE_USNT,
	SWPORTxCTL_MODE_DS, SWPORTxCTL_MODE_DS, SWPORTxCTL_MODE_NT,
 ...
 */
static const uint32_t swportpart[IDT_PORTCNT] = {0, 0, 0, 1, 2, 3, 4, 5};

/*! @var swport
 *   IDT PCIe-switch port device numnber
 */
static const uint32_t swportdevnum[IDT_PORTCNT] = {0, 2, 4, 0, 0, 0, 0, 0};
 ...
 */
void eeimg_cncbp_p4ds(struct eeparams *params)
{
	...
	/* Initialize the port partitions 0-5 and wait until the change is done, then
	 * clear the status register */
	for (idx = 0; idx < (IDT_PARTCNT - 2); idx++) {
	 ...
	 * Port 0(US+NT), 2(DS), 4(DS) belong to partition 0
	 * Ports 6 - 20 (NT) respectively belong to the partitions 1 - 5
	...
	for (idx = 0; idx < IDT_PORTCNT; idx++) {
		/* Skip port 2 and 4 since they have Downstream function enabled only */
		if (idx == 1 || idx == 2)
			continue;
